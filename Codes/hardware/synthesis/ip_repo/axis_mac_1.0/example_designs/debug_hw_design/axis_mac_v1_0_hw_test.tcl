# Runtime Tcl commands to interact with - axis_mac_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/axis_mac_v1_0_include.tcl
...
# Test: S_AXIS_DataIn
# Create a write transaction at s_axis_datain_addr address
create_hw_axi_txn w_s_axis_datain_addr [get_hw_axis $jtag_axi_master] -type write -address $s_axis_datain_addr -data $wdata_1
# Create a read transaction at s_axis_datain_addr address
create_hw_axi_txn r_s_axis_datain_addr [get_hw_axis $jtag_axi_master] -type read -address $s_axis_datain_addr
# Initiate transactions
run_hw_axi r_s_axis_datain_addr
run_hw_axi w_s_axis_datain_addr
run_hw_axi r_s_axis_datain_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_s_axis_datain_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - S_AXIS_DataIn"
} else {
	puts "Data comparison test fail for - S_AXIS_DataIn, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: M_AXIS_DataOut
set wdata_m_axis_dataout [get_init_data 0]
create_hw_axi_txn w_m_axis_dataout_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_m_axis_dataout}
create_hw_axi_txn r_m_axis_dataout_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_m_axis_dataout_addr
run_hw_axi w_m_axis_dataout_addr
run_hw_axi r_m_axis_dataout_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_m_axis_dataout_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - M_AXIS_DataOut"
} else {
	puts "Data comparison test fail for - M_AXIS_DataOut, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
